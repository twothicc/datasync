...
func NewSyncMessageHandler(ctx context.Context, esClient *elastic.Client) (kafkahandler.IMessageHandler, error) {
	if esClient == nil {
		return nil, ErrConstructor.New("[NewSyncMessageHandler]elasticsearch client cannot be nil")
	}

	smh := &SyncMessageHandler{
		ctx:         ctx,
		esClient:    esClient,
