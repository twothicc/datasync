...
type IMessageConsumer interface {
	ConsumeTopics(ctx context.Context, topics []string, handler kafkahandler.IMessageHandler)
}
...
func (mc *MessageConsumer) ConsumeTopics(ctx context.Context, topics []string, handler kafkahandler.IMessageHandler) {
	go func() {
		for {
